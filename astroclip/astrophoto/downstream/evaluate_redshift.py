...
if __name__ == "__main__":
    ...
    parser.add_argument("--embedding_dir", type=str, default=f"{ASTROCLIP_ROOT}/pretrained/embeddings/astrophoto",
                        help="Directory containing the .npz embedding files.")
    parser.add_argument("--output_dir", type=str, default=f"{ASTROCLIP_ROOT}/pretrained/results/astrophoto",
                        help="Directory to save plots and results.")
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)

    log_file_path = os.path.join(args.output_dir, f"{args.model_name}_evaluation_log.txt")

    with open(log_file_path, 'w') as log_file:
        print(f"Logs will be saved to: {log_file_path}")
        log_file.write(f"--- Redshift Prediction Log for '{args.model_name}' Embeddings ---\n\n")

        train_path = os.path.join(args.embedding_dir, f"train_{args.model_name}_embedding.npz")
        test_path = os.path.join(args.embedding_dir, f"test_{args.model_name}_embedding.npz")
        ...
        evaluate_and_plot(y_test, y_pred_mlp, f"{args.model_name}_MLP", args.output_dir, log_file)
        ...
        evaluate_and_plot(y_test, y_pred_knn, f"{args.model_name}_KNN", args.output_dir, log_file)

        visualize_with_tsne(X_test_scaled, y_test, args.model_name, args.output_dir)

    print(f"\nAll tasks complete. Results are in '{args.output_dir}'.")