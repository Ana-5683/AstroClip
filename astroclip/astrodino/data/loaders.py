...
def str_to_bool(s):
    ...
    return mapping.get(s.strip().lower(),  None)  # 非匹配值返回None
...
def _parse_dataset_str(dataset_str: str):
    ...
    for token in tokens[1:]:
        ...
        assert key in ("root", "extra", "split","extinction","probs")
        kwargs[key] = value

    if name == "QuasarDataset":
        class_ = QuasarDataset
        if "split" in kwargs:
            kwargs["split"] = kwargs["split"]
        if "extinction" in kwargs:
            kwargs["extinction"] = str_to_bool(kwargs["extinction"])
        if "probs" in kwargs:
            kwargs["probs"] = str_to_bool(kwargs["probs"])
    else:
...
def _make_sampler(
    *,
    ...
    if sampler_type == SamplerType.INFINITE:
        ...
        use_new_shuffle_tensor_slice = sampler_type == SamplerType.SHARDED_INFINITE_NEW
