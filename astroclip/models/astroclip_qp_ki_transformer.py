...
class ImagePhotoHead(nn.Module):
    def __init__(
        ...
        self.image_proj=nn.Linear(image_embed_dim, fusion_dim)
        self.photo_proj=nn.Linear(photo_embed_dim, fusion_dim)

        # 3. 核心组件: Cross-Attention (Q=Photo, K=Image, V=Image)
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=fusion_dim,
            num_heads=n_head,
            dropout=dropout,
            batch_first=True,
            # kdim = image_embed_dim,  # key 输入维度（image backbone 输出 dim）
            # vdim = image_embed_dim,  # value 输入维度（同 key）
        )

        # Add & Norm  (标准的 Transformer Block 结构)
        self.laynorm1 = nn.LayerNorm(fusion_dim)
        self.laynorm2 = nn.LayerNorm(fusion_dim)

        self.ffn = MLP(
            in_features=fusion_dim,
            hidden_features=4 * fusion_dim,
            dropout=dropout,
            out_features=clip_embed_dim
        )

        # 4. Final Projector to CLIP Space
        # 输入是 fusion_dim (768), 输出是 embed_dim (1024)
        self.clip_projector = MLP(
            in_features=fusion_dim,
            hidden_features=4 * fusion_dim,
            out_features=clip_embed_dim,
            dropout=dropout
    ...
    def forward(self, image, photometry):
        ...
        # [Batch, 1, 768] (变为序列形式)
        photo_features = photo_features.unsqueeze(1)

        img_features = self.image_proj(img_features)
        photo_features = self.photo_proj(photo_features)
        ...
        # 残差连接 1: Photo + Denoised_Image
        # 这一步就是你说的“融合”
        fused_feat = self.laynorm1( photo_features + attn_out)

        # FFN + 残差连接 2
        fused_feat = self.laynorm2(fused_feat + self.ffn(fused_feat))

        # fused_feat shape: [Batch, 1, 768]
        # --- 4. 投影到 CLIP 空间 ---
        # output: [Batch, 1024]
        clip_out = self.clip_projector(fused_feat).squeeze(1)
