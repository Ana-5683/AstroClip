...
class SpecFormer(L.LightningModule):
    ...
    def forward_without_preprocessing(self, x: Tensor):
        ...
        t = x.shape[1]
        if t > self.hparams_['max_len']:
            raise ValueError(
                f"Cannot forward sequence of length {t}, "
                f"block size is only {self.hparams_['max_len']}"
            )
    ...
    def training_step(self, batch):
        ...
        self.log("training_loss", loss, prog_bar=True,sync_dist=True)
        return loss
    ...
    def validation_step(self, batch):
        ...
        locs = (input != target).type_as(output)
        loss = F.mse_loss(output * locs, target * locs, reduction="mean") / locs.mean()
        self.log("val_training_loss", loss, prog_bar=True,sync_dist=True)
        return loss
    ...
    def _reset_parameters_datapt(self):
        ...
        for emb in [self.data_embed, self.position_embed]:
            std = 1 / math.sqrt(self.hparams_['embed_dim'])
            nn.init.trunc_normal_(emb.weight, std=std, a=-3 * std, b=3 * std)

        # transformer block weights
        self.blocks.apply(lambda m: _init_by_depth(m, self.hparams_['num_layers']))
        self.head.apply(lambda m: _init_by_depth(m, 1 / 2))

    def _slice(self, x):
        start_indices = np.arange(
            0,
            x.shape[1] - self.hparams_['slice_overlap'],
            self.hparams_['slice_section_length'] - self.hparams_['slice_overlap'],
        )
        sections = [
            x[:, start : start + self.hparams_['slice_section_length']].transpose(1, 2)
            for start in start_indices
        ]

        # If the last section is not of length 'section_length', you can decide whether to keep or discard it
        if sections[-1].shape[1] < self.hparams_['slice_section_length']:
    ...
    def _mask_seq(self, seq: torch.Tensor) -> torch.Tensor:
        ...
        len_ = seq.shape[0]
        num_chunks = self.hparams_['mask_num_chunks']
        chunk_width = self.hparams_['mask_chunk_width']
