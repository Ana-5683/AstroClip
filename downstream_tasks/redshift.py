...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True,
                        choices=["astroclip_image", "astroclip_spectrum", "astroclip_photo", "astroclip_ip",
                                 "astrodino", "specformer"],
                        help="用于生成嵌入的模型名称。")
    ...
    pretrained_dir = f"{ASTROCLIP_ROOT}/pretrained/embeddings/{args.ckpt}"
    output_dir = f"{ASTROCLIP_ROOT}/pretrained/results/{args.ckpt}"
    if args.model in ["astroclip_image", "astroclip_spectrum", "astroclip_photo", "astroclip_ip"]:
        output_dir = os.path.join(output_dir, args.model)

    os.makedirs(output_dir, exist_ok=True)
    # 定义日志文件路径
    log_file_path = os.path.join(output_dir, f"{args.model}_log.txt")
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        if args.model in ["astroclip_image", "astroclip_spectrum", "astroclip_photo", "astroclip_ip"]:
            train_embedding_path = os.path.join(pretrained_dir, args.model, f"train_{args.model}_embedding.npz")
            test_embedding_path = os.path.join(pretrained_dir, args.model, f"test_{args.model}_embedding.npz")
        else:
            train_embedding_path = os.path.join(pretrained_dir, f"train_{args.model}_embedding.npz")
            test_embedding_path = os.path.join(pretrained_dir, f"test_{args.model}_embedding.npz")
        ...
        evaluate_and_plot(y_test, y_pred_mlp, f"{args.model}_MLP", output_dir, log_file)
        ...
        # 使用还原后的预测值和原始的 y_test 进行评估
        evaluate_and_plot(y_test, y_pred_knn, f"{args.model}_KNN", output_dir, log_file)

        # 3. t-SNE 可视化 (保持不变)
        visualize_with_tsne(X_test_scaled, y_test, args.model, output_dir)

    print(f"\n所有任务完成。结果已保存至 '{output_dir}' 目录。")
